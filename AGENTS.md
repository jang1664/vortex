...
- Current branch target: `fpint_naive`.
- Treat `fpint_gemm_ffn_hw` as the default FPINT GEMM regression app for this branch.
- `fpint_gemm_ffn_hw_improve` and instruction-stream terminology belong to the `fpint_improve` flow. Do not use them by default unless the user explicitly asks for that variant.
...
- For FPINT GEMM blackbox runs on this branch, pass `--app fpint_gemm_ffn_hw` if a wrapper default points elsewhere.

## Codex Skills

- Project skill sources live under `harness/skills`.
- Claude exposes them through `.claude/skills` symlinked to `harness/skills`.
- In Codex, when a project skill is referenced, read and follow `harness/skills/<skill-name>/SKILL.md` directly.
