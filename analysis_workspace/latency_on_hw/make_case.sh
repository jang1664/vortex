...
fpga_bin_remaps_csv="${FPGA_BIN_REMAPS:-${FPGA_BIN_REMAP:-C4=C4_v3,C3=C3}}"
...
if [[ "$target" == "full" ]]; then
  ./make_cases.sh \
    ...
    --output generated_suites/llama2_7b_main_full_v2.C3_C4_v3 \
    --prefill-batches 1 \
  ...
  ./make_cases.sh \
    ...
    --output generated_suites/llama3_8b_main_full_v2.C3_C4_v3 \
    --prefill-batches 1 \
    --prefill-seq-lens 1024,2048,4096,8192,16384,32768 \
    --generation-batches 1,4,64 \
    --generation-seq-lens 1024,2048,4096,8192,16384,32768 \
