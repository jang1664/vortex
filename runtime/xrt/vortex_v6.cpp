...
public:
  ...
  #ifdef CPP_API
    xrtBufferMaps_.reserve(num_banks);
  #endif
    for (uint32_t i = 0; i < num_banks; ++i) {
    #ifdef CPP_API
      xrtBuffers_.emplace_back(xrtDevice_, bo_size_, xrt::bo::flags::normal, i);
      auto mapped_ptr = xrtBuffers_.back().map<uint8_t*>();
      if (mapped_ptr == nullptr) {
        fprintf(stderr, "[VXDRV] Error: failed to map bank %u BO\n", i);
        return -1;
      }
      xrtBufferMaps_.push_back(mapped_ptr);
    #else
...
#if defined(VX_USE_BANKED_XRT_BO) && defined(CPP_API)
  struct bank_transfer_span_t {
    uint64_t begin = 0;
    uint64_t end = 0;
    bool used = false;
  };

  int extend_bank_span(bank_transfer_span_t *span,
                       uint32_t bank,
                       uint64_t offset,
                       uint64_t size) const {
    if (!span->used) {
      span->begin = offset;
      span->end = offset + size;
      span->used = true;
      return 0;
    }
    if (offset != span->end) {
      fprintf(stderr,
              "[VXDRV] Error: non-contiguous bank span: bank=%u "
              "expected=0x%lx actual=0x%lx\n",
              bank, span->end, offset);
      return -1;
    }
    span->end += size;
    return 0;
  ...
  int collect_bank_spans(
      uint64_t dev_addr,
      uint64_t size,
      std::vector<bank_transfer_span_t> *spans) const {
    uint64_t remaining = size;
    while (remaining != 0) {
      uint32_t bank;
      uint64_t offset;
      CHECK_ERR(this->get_bank_info(dev_addr, &bank, &offset), {
        return err;
      });
      uint64_t chunk = (remaining > CACHE_BLOCK_SIZE)
                     ? CACHE_BLOCK_SIZE : remaining;
      if (bank >= spans->size() || offset + chunk > bo_size_) {
        fprintf(stderr,
                "[VXDRV] download oob while collecting spans: "
                "dev_addr=0x%lx bank=%u off=0x%lx "
                "xfer=0x%lx bo_size=0x%lx\n",
                dev_addr, bank, offset, chunk, bo_size_);
        return -1;
      }
      CHECK_ERR(this->extend_bank_span(
          &spans->at(bank), bank, offset, chunk), {
        return err;
      });
      dev_addr += chunk;
      remaining -= chunk;
...
#if defined(VX_USE_BANKED_XRT_BO) && defined(CPP_API)
    std::vector<bank_transfer_span_t> spans(xrtBuffers_.size());
#endif
    uint64_t remaining = size;
    while (remaining != 0) {
      uint32_t bo_index;
...
#if defined(VX_USE_BANKED_XRT_BO) && defined(CPP_API)
      std::memcpy(xrtBufferMaps_.at(bo_index) + bo_offset,
                  host_ptr, xfer_size);
      CHECK_ERR(this->extend_bank_span(
          &spans.at(bo_index), bo_index, bo_offset, xfer_size), {
        return err;
      });
...
#if defined(VX_USE_BANKED_XRT_BO) && defined(CPP_API)
    for (uint32_t bank = 0; bank < spans.size(); ++bank) {
      const auto& span = spans.at(bank);
      if (!span.used)
        continue;
      xrtBuffers_.at(bank).sync(
          XCL_BO_SYNC_BO_TO_DEVICE, span.end - span.begin, span.begin);
    }
#endif
...
#if defined(VX_USE_BANKED_XRT_BO) && defined(CPP_API)
    std::vector<bank_transfer_span_t> spans(xrtBuffers_.size());
    CHECK_ERR(this->collect_bank_spans(dev_addr, size, &spans), {
      return err;
    });
    for (uint32_t bank = 0; bank < spans.size(); ++bank) {
      const auto& span = spans.at(bank);
      if (!span.used)
        continue;
      xrtBuffers_.at(bank).sync(
          XCL_BO_SYNC_BO_FROM_DEVICE, span.end - span.begin, span.begin);
    }
#endif
    uint64_t remaining = size;
    while (remaining != 0) {
...
#if defined(VX_USE_BANKED_XRT_BO) && defined(CPP_API)
      std::memcpy(host_ptr,
                  xrtBufferMaps_.at(bo_index) + bo_offset, xfer_size);
#elif defined(CPP_API)
...
#ifdef CPP_API
  std::vector<uint8_t*> xrtBufferMaps_;
#endif
