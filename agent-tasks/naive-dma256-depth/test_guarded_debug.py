...
for lanes,bytes_,depth,masked,seed,reorder,inject,tagbits in matrix:
 ...
 out=BUILD/'split_guarded_debug'/name;out.mkdir(parents=True,exist_ok=True)
 cmd=['vcs','-full64','-sverilog','-timescale=1ns/1ps','+define+SIMULATION','+define+XLEN_64','+define+DBG_TRACE_MEM','+define+GEMM_NAIVE',f'+incdir+{rtl}','+libext+.sv','-y',str(rtl/'libs'),'-y',str(rtl/'mem'),str(rtl/'VX_gpu_pkg.sv'),str(rtl/'mem/VX_mem_bus_if.sv'),str(TASK/'final_candidate/VX_mem_bus_split.sv'),str(tb),'-top','tb_mem_bus_split_depth']
 ...
 (TASK/'split_guarded_debug_results.json').write_text(json.dumps(results,indent=2)+'\n')
