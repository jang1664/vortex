...
OUT=TASK/'p5-verification/occupied-reset-iteration1';OUT.mkdir(parents=True,exist_ok=False)
BUILD=ROOT/'build_naive_input_contexts_verify';assert (BUILD/'config.mk').exists()
directory=BUILD/'hw/unittest/naive_qparam_dma'
for name in ('Makefile','vcs.mk'):shutil.copy2(ROOT/'hw/unittest/naive_qparam_dma'/name,directory/name)
...
for mx in (16,32):
 ...
 cmd=['python3',str(ROOT/'tools/verify_rtl.py'),'unittest','--path',str(directory),'--sim','vcs','--timeout','600','--extra-sim-args=+RESET_ONLY']
 ...
 if result.returncode==0:
  simlog=(case/'sim.log').read_text();assert 'JOINT_RESET_FLUSHED' in simlog and 'CASE PASS' not in simlog
