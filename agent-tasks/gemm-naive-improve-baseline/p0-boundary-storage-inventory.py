...
if __name__ == "__main__":
    ...
    assert expected == (TASK/"p0-boundary-storage-node.sv").read_text(), "Stale or non-equivalent snapshot"

    for mxu in (16,32):
        result=inventory(BUILD/f"boundary{mxu}.xml",mxu)
        result["original_node_sha256"] = hashlib.sha256(original.read_bytes()).hexdigest()
        (TASK/f"p0-boundary-storage-elaborated{mxu}.json").write_text(json.dumps(result,indent=2)+"\n")
