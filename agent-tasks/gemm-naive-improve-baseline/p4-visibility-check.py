...
def paths():
    ...
    for n in ("gemm_wr_lane_pending_r", "gemm_done_drained",
              "gemm_wr_lane_push", "gemm_wr_lane_pop"):
        p[n] = NODE + "/" + n
    p["gemm_done_drained"] = NODE + "/input_executor/contexts/done_valid"
    p["terminal"] = NODE + "/input_executor/contexts/done_terminal"
    p["done_ready"] = NODE + "/input_executor/contexts/done_ready"
    p["store_done"] = NODE + "/output_store_done"
    p["cfg"] = NODE + "/cfg_start_fire"
...
def audit(signals):
    ...
    for edge, time, v in sample(signals):
        ...
        if number(v, "gemm_done_drained") and number(v, "terminal") and number(v, "done_ready"):
        ...
        previous_afu = afu
    assert accepted == committed == 3968 * 8 + 128 * 4 and not any(pending.values())
    assert reads["PBUF"] == 3968 * 8 and reads["OBUF"] == 512
    assert all(x["bank_writes_pending"] == 0 for x in drain_pending)
    assert all(x["pending_bank_writes"] == 0 for x in store_events)
    assert len(afu_events) == 1
    assert not read_violations, read_violations[:3]
    assert len(store_events) == 4 and len(drain_pending) == 4
...
def main():
    ...
    service = json.loads((args.run / "service/result.json").read_text())
    assert service["wave_sha256"] == wave_sha
    commands = json.loads((args.run / "service/commands.json").read_text())
    # Service checker independently validates every live command/input/writeback
    # address against the immutable canonical geometry in this same wave.
    assert min(c["descriptor"]["final_base"] for c in commands) == 0x15000
    assert min(c["descriptor"]["psum_base"] for c in commands) == 0x1d000
    assert all(0x15000 <= c["descriptor"]["final_base"] < 0x1d000 and
               0x1d000 <= c["descriptor"]["psum_base"] < 0x2d000 for c in commands)
    snapshot = args.run / "physical-visibility-signals.json"
    if snapshot.exists():
    ...
    for filename in (
        "hw/rtl/core/gemm/VX_naive_external_dma_executor.sv", "hw/rtl/core/gemm/VX_gemm_node_naive.sv",
    ...
    (args.run / "physical-visibility-results.json").write_text(json.dumps(result, indent=2) + "\n")
