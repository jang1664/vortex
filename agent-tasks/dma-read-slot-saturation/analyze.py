...
def sample(name, path):
    cache = out / (name + '.npz')
    if cache.exists() and not name.endswith('rd_state'):
        a = np.load(cache)
        t, v = a['t'], a['v']
    else:
...
assert len(input_edges) == manifest['m'] * 1024
assert len(acc_edges) == manifest['m'] * 1024
...
result = dict(backend=manifest['backend'], m=manifest['m'], depth=args.depth,
              ...
              phases=phases, phase_activity=breakdown, dma_units=dma_units, node_stalls=node_stalls,
