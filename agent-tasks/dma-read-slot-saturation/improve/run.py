#!/usr/bin/env python3
"""Measure improve DMA read capacity at M4 and M256 with FSDB."""
import argparse
...
def main():
    results = []
    for m in (4, 256):
        out = TASK / 'runs' / f'depth{args.depth}-m{m}'
        cmd = [sys.executable, str(RUNNER), 'improve', '--m', str(m),
               '--k', '512', '--n', '512', '--qdir', '0', '--wtrans', '0',
               '--repeat', '1', '--timeout', '7200', '--build', str(BUILD),
               '--output', str(out), '--config', str(TASK / f'depth{args.depth}.sh')]
        if m == 4:
            cmd.append('--rebuild')
        print(f'Starting depth {args.depth} M{m}', flush=True)
        ...
        result = dict(dma_read_slots=args.depth, perf=manifest.get('perf', []), backend='improve', micro_tile_order='N-fast', m=m, k=512, n=512, runner_returncode=run.returncode,
        ...
        result['passed'] = (run.returncode == 0 and result['wrapper_returncode'] == 0
                            ...
                            and (out / 'wave.fsdb').is_file()
                            and (out / 'wave.fsdb').stat().st_size > 0)
        if not result['passed']:
            result['error_excerpt'] = verify.extract_errors(combined)
        (out / 'result.json').write_text(json.dumps(result, indent=2) + '\n')
        results.append(result)
        (TASK / 'runs' / f'depth{args.depth}-results.json').write_text(json.dumps(results, indent=2) + '\n')
