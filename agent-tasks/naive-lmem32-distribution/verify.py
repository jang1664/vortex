#!/usr/bin/env python3
"""Measure one frozen naive LMEM topology with VCS and FSDB."""
import argparse
import hashlib
...
def main():
    ...
    parser.add_argument('--topology', choices=['p16_b16', 'p32_b16', 'p16_b32', 'p32_b32'], required=True)
    ...
    build = ROOT / f'build_naive_lmem32_{args.topology}_vcs'
    config = (args.config or TASK / 'configs' / f'{args.topology}.sh').resolve()
    out = TASK / 'runs' / args.iteration / args.topology / args.case
    if out.exists():
    ...
    if args.rebuild:
        cmd.append('--rebuild')
    print(f'Starting {args.topology} {args.case}: {out}', flush=True)
    ...
    wave = out / 'wave.fsdb'
    result = dict(topology=args.topology, pipelined_input_issue=True, read_quota=1, case=args.case, m=m, k=k, n=n,
