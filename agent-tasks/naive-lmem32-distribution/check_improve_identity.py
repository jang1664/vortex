...
NEW=ROOT/'hw/rtl';OLD=TASK/'runs/improve-identity/before-rtl';OUT=TASK/'runs/improve-identity';OUT.mkdir(parents=True,exist_ok=True)
baseline_files=json.loads((TASK/'baseline.json').read_text())['files']
missing=[f for f in baseline_files if not (TASK/'baseline-rtl'/f).is_file()]
...
for src in NEW.rglob('*'):
 if src.is_file():
  ...
  baseline=TASK/'baseline-rtl/hw/rtl'/src.relative_to(NEW)
  target=baseline if baseline.exists() else src
  if not dst.exists():dst.symlink_to(target.resolve())
flags=shlex.split(subprocess.check_output(['bash','-c','source "$1"; printf "%s" "$CONFIGS"','bash',str(ROOT/'configs/improve_th16_tcol16_hwexp_dcache_sxbar_f16_bigmem.sh')],cwd=ROOT,text=True))
