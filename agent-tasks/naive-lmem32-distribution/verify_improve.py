#!/usr/bin/env python3
"""Verify improve GEMM/core cycle identity after naive-only changes."""
import argparse
import hashlib
...
def main():
    ...
    parser.add_argument('--config', type=Path, default=ROOT / 'agent-tasks/dma-read-slot-saturation/improve/depth16.sh')
    parser.add_argument('--case', choices=['m4', 'm256'], required=True)
    parser.add_argument('--iteration', default='v3')
    parser.add_argument('--rebuild', action='store_true')
    parser.add_argument('--timeout', type=int, default=7200)
    args = parser.parse_args()
    m, k, n = {'m4': (4, 512, 512), 'm256': (256, 512, 512)}[args.case]
    build = ROOT / 'build_dma_slots_improve_vcs'
    config = args.config.resolve()
    out = TASK / 'runs' / args.iteration / 'improve' / args.case
    if out.exists():
        parser.error(f'Refusing to overwrite existing evidence: {out}')
    config_hash = hashlib.sha256(config.read_bytes()).hexdigest()
    cmd = [sys.executable, str(RUNNER), 'improve', '--m', str(m), '--k', str(k),
           '--n', str(n), '--qdir', '0', '--wtrans', '0', '--repeat', '1',
           '--timeout', str(args.timeout), '--build', str(build), '--output', str(out),
           '--config', str(config)]
    if args.rebuild:
        cmd.append('--rebuild')
    print(f'Starting improve identity {args.case}: {out}', flush=True)
    ...
    wave = out / 'wave.fsdb'
    result = dict(backend="improve", case=args.case, m=m, k=k, n=n,
    ...
    expected = {'m4': (6431, 12205), 'm256': (272856, 278622)}[args.case]
    result['expected_gemm_cycles'], result['expected_core_cycles'] = expected
    result['zero_cycle_delta'] = gemm == [expected[0]] and core == [expected[1]]
    result['passed'] = result['passed'] and result['zero_cycle_delta']
