#!/usr/bin/env python3
"""Measure naive weight8 with default PSUM16 at M4 and M256."""
import argparse
...
parser.parse_args()
BUILD = ROOT / 'build_naive_weight8_vcs'
...
def main():
    results = []
    for m in (4, 256):
        out = TASK / 'runs' / f'naive-m{m}'
        cmd = [sys.executable, str(RUNNER), 'naive', '--m', str(m),
               '--k', '512', '--n', '512', '--qdir', '0', '--wtrans', '0',
               '--repeat', '1', '--timeout', '5400', '--build', str(BUILD),
               '--output', str(out), '--config', str(ROOT / 'configs/naive_gemm_th16_tcol16_hwexp_dcache_sxbar_f16.sh')]
        if m == 4:
            cmd.append('--rebuild')
        ...
        result = dict(weight_slots=8, psum_read_slots=16, psum_response_slots=16, m=m, k=512, n=512, runner_returncode=run.returncode,
