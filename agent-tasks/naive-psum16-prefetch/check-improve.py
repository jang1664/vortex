...
OUT = Path(__file__).resolve().parent / 'runs' / 'improve-identity'
OUT.mkdir(parents=True, exist_ok=False)
changed = subprocess.check_output(
    ['git', 'diff', 'HEAD', '--name-only', '--', 'hw/rtl'], cwd=ROOT,
    text=True).splitlines()
assert set(changed) == {
    'hw/rtl/core/gemm/VX_gemm_acc_lmem.sv',
    'hw/rtl/core/gemm/VX_gemm_node_naive.sv',
}, changed
config = subprocess.check_output(
    ['bash', '-c', 'source agent-tasks/fpint-gemm-latency-compare/improve.sh; '
