#!/usr/bin/env python3
"""Build the comparison from successful, numerically checked RTL runs."""
import json, re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]
OUT=ROOT/'docs/hw_analysis/improve_vs_naive'
data={}
for name in ['improve','naive']:
    manifest=json.loads((OUT/f'logs/vcs_{name}/results.json').read_text())
    assert len(manifest['runs']) == 2
    for row in manifest['runs']:
        assert row['returncode'] == 0 and row['passed'], row
        perf='\n'.join(row['perf'])
        gemm=int(re.search(r'total_cycles=(\d+)',perf)[1])
        kernel=int(re.search(r'PERF: instrs=\d+, cycles=(\d+)',perf)[1])
        overlap=float(re.search(r'DMA\+MXU overlap=([\d.]+)%',perf)[1])
        data[name,row['M']]={'gemm':gemm,'kernel':kernel,'overlap':overlap}
lines=['# FPINT GEMM latency: improve vs naive, th16 / MXU16x16','',
...
for m in [4,256]:
    i,n=data['improve',m],data['naive',m]
    lines.append(f'- **M={m}, K=512, N=512:** improve is **{n["gemm"]/i["gemm"]:.3f}x faster** over the GEMM interval, reducing latency by **{100*(1-i["gemm"]/n["gemm"]):.2f}%**. Whole-kernel speedup is **{n["kernel"]/i["kernel"]:.3f}x**.')
lines+=['','## GEMM interval','',
...
'|---:|---:|---:|---:|---:|---:|---:|---:|---:|']
for m in [4,256]:
    i,n=data['improve',m]['gemm'],data['naive',m]['gemm']
    lines.append(f'| {m} | 512 | 512 | {n:,} | {i:,} | {n/100:.2f} | {i/100:.2f} | {n/i:.3f}x | {100*(1-i/n):.2f}% |')
lines+=['','Speedup = naive cycles / improve cycles. Reduction = (naive cycles - improve cycles) / naive cycles.','',
'## Whole kernel','',
...
'|---:|---:|---:|---:|---:|---:|---:|']
for m in [4,256]:
    i,n=data['improve',m]['kernel'],data['naive',m]['kernel']
    lines.append(f'| {m} | {n:,} | {i:,} | {n/100:.2f} | {i/100:.2f} | {n/i:.3f}x | {100*(1-i/n):.2f}% |')
lines+=['','## Conditions and interpretation','',
...
'|---:|---:|---:|']
for m in [4,256]:
    lines.append(f'| {m} | {data["naive",m]["overlap"]:.3f}% | {data["improve",m]["overlap"]:.3f}% |')
lines+=['',
'The logged overlap is overlap cycles divided by DMA-active cycles, not a percentage of total GEMM time. These percentages are not directly comparable: VX_core ties the naive local-DMA performance structs to zero, while improve includes local operand/output DMA activity in the DMA-active union. The [M4 FSDB analysis](fpint_gemm_m4_bandwidth_analysis.md) compares explicitly selected external-DMA busy signals and common MXU input handshakes instead. It finds faster external transfers together with a large reduction in repeated command startup and completion gaps. The [root-cause follow-up](fpint_gemm_naive_input_gap_root_cause.md) traces those gaps to the generic DMA startup, registered LMEM path, and per-command writeback/synchronization fences.',
'',
...
(OUT/'fpint_gemm_latency.md').write_text('\n'.join(lines))
print('Wrote',OUT/'fpint_gemm_latency.md')
