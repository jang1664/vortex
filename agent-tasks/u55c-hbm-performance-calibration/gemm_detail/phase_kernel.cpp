// Diagnostic copy of original kernel; original source is unchanged.
...
static bool run_gemm_job_once(kernel_arg_t* arg, const tb_partition_t& part, bool reporter) {
  ...
  const uint64_t before_program = csr_read(VX_CSR_MCYCLE);
  asm volatile ("" ::: "memory");
  program_job_regs(eid, arg, part);
  asm volatile ("" ::: "memory");
  const uint64_t after_program = csr_read(VX_CSR_MCYCLE);

  uint32_t last_ctrl = 0;
  wait_job_done(eid, generation, last_ctrl);
  asm volatile ("" ::: "memory");
  const uint64_t after_wait = csr_read(VX_CSR_MCYCLE);
  if (reporter) {
    auto detail = reinterpret_cast<volatile phase_args_t*>(arg);
    detail->before_program = before_program;
    detail->after_program = after_program;
    detail->after_wait = after_wait;
  }
  return true;
}
...
int main() {
  ...
  if (vx_warp_id() == 0 && vx_thread_id() == 0) {
    auto detail = reinterpret_cast<volatile phase_args_t*>(csr_read(VX_CSR_MSCRATCH));
    detail->entry_cycle = entry;
    detail->return_cycle = returned;
  }
  return result;
}
