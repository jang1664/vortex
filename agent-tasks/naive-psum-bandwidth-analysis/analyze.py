...
RUNS={'improve4':'agent-tasks/gemm-naive-improve-baseline/p4-candidate/improve-m4-iteration1',
'improve256':'agent-tasks/gemm-naive-improve-baseline/p4-candidate/improve-m256-iteration1',
'naive4':'agent-tasks/naive-psum-bandwidth-analysis/runs/naive-m4',
'naive256':'agent-tasks/naive-psum-bandwidth-analysis/runs/naive-m256'}
def analyze(name):
 ...
 if naive:
  for k in ['psum_rd_order_block','psum_rd_pending_conflict','psum_rd_current_conflict']:paths[k]=node+'/'+k
 samples={};missing=[]
 def capture(item):
  key,path=item
  cache=out/(key+'.npz')
  if cache.exists():
   ...
   if not r.data_rows:missing.append(key);return
 ...
 with ThreadPoolExecutor(max_workers=4) as signal_pool:list(signal_pool.map(capture,paths.items()))
 ...
 'post_psum_only_block':(s['post_txn_count']>0)&(s['post_head_scaled_ready']==1)&(s['post_head_psum_ready']==0)&((s['acc_result_credit']>0)|(s['acc_result_commit']==1)),
 'input_admission_block':iv&(s['input_admission_ready']==0),
 ...
 if naive:
  for k in ['psum_rd_order_block','psum_rd_pending_conflict','psum_rd_current_conflict']:masks[k]=s[k]==1
