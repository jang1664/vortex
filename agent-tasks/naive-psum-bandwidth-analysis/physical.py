...
def run(name):
 ...
 log_path=source/'simv.log'
 if not log_path.exists():log_path=BASE/'build_naive_psum32_vcs/sim/xrtsim_vcs/simv.log'
 log=next(l for l in log_path.read_text().splitlines() if l.startswith(('GEMM_LATENCY_DONE ','GEMM_LATENCY_CFG ')));cfg=int(re.search(r'e_cfg=(\d+)',log)[1])
 ...
 for k in ['input_fire','post_txn_launch','post_txn_count','post_head_psum_ready','post_head_scaled_ready','weight_ready','compute_fire','prealigner_out_valid','pre_meta_valid_out']:
  paths['compute_'+k]=node+'/u_VX_gemm_compute_core/'+k
 for k in ['psum_rd_pending_conflict','psum_rd_current_conflict','psum_rd_order_block']:
  paths[k]=node+'/'+k
 for bus in ['psum_rd_raw_bus_if','psum_wr_raw_bus_if']:
  paths[bus+'_addr']=node+'/'+bus+'/req_data.addr'
 samples={};missing=[]
 ...
 r['raw_read_stall']=count((s['psum_rd_raw_bus_if_req_valid']==1)&(s['psum_rd_raw_bus_if_req_ready']==0))
 r['read_order_block']=count(s['psum_rd_order_block']==1)
 r['read_pending_write_block']=count(s['psum_rd_pending_conflict']==1)
 r['read_current_write_block']=count(s['psum_rd_current_conflict']==1)
 r['current_conflict_different_address']=count((s['psum_rd_current_conflict']==1)&(s['psum_rd_raw_bus_if_addr']!=s['psum_wr_raw_bus_if_addr']))
 r['input_fire']=count(s['compute_input_fire']==1)
 r['post_wait_psum']=count((s['compute_post_txn_count']>0)&(s['compute_post_head_psum_ready']==0))
 r['weight_wait']=count((s['compute_prealigner_out_valid']==1)&(s['compute_pre_meta_valid_out']==1)&(s['compute_weight_ready']==0))
...
if __name__=='__main__':
 with ThreadPoolExecutor(max_workers=2) as p:list(p.map(run,sys.argv[1:] or ['naive4','naive256']))
