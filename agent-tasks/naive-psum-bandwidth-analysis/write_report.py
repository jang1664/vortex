"""Write the post-prefetch bandwidth attribution from completed measurements."""
import json
...
OUT=ROOT/'docs/hw_analysis/improve_vs_naive/fpint_gemm_post_prefetch_bandwidth.md'
def read(name,file):return json.loads((TASK/'captures'/name/file).read_text())
names=['improve4','naive4','improve256','naive256']
r={n:read(n,'result.json') for n in names}
mem={n:read(n,'memory/result.json') for n in names}
physical=read('naive256','physical/result.json')
weight4=read('naive4','weight/result.json')
weight256=read('naive256','weight/result.json')
def number(x):return f'{x:,}'
def measured_table(keys):
    lines=[]
    for label,key in keys:
        lines.append('| '+label+' | '+' | '.join(number(r[n]['counts'][key]) for n in names)+' |')
    return '\n'.join(lines)
rows='\n'.join(f"| {m} | {number(r['improve'+str(m)]['cycles'])} | {number(r['naive'+str(m)]['cycles'])} | {r['naive'+str(m)]['cycles']/r['improve'+str(m)]['cycles']:.3f}x |" for m in [4,256])
text=f'''# Post-prefetch M4/M256 speedup: bandwidth attribution

The larger improve speedup at M4 is consistent with exposed weight delivery overhead, but it does **not** isolate a pure memory-bandwidth advantage. Current naive M4 is limited by its LMEM weight-gather path, including a four-slot response queue. Current naive M256 still exposes partial-sum read latency, including conservative bank-set write/read ordering. A PSUM capacity plateau is not evidence that memory bandwidth is saturated.
...
|---|---:|---:|---:|---:|
{measured_table([('Input valid but not ready','input_backpressure'),('Operands waiting for required weight version','compute_wait_weight'),('PSUM core read request valid but not ready','acc_rd_req_stall'),('PSUM is the remaining postprocessing blocker','post_psum_only_block'),('Input admission blocked by accumulator capacity','input_admission_block'),('External DMA busy, union of active cycles','external_busy')])}
...
Naive has {number(weight4['fetch_blocked_no_slot'])} cycles with a pending weight fetch but no available response slot. Its four slots are full for {number(weight4['queue_slot_hist']['4'])} cycles. Of these fetch-blocked cycles, 11767 overlap compute-side weight waiting. Improve has eight weight slots and **zero** pending-fetch/no-slot cycles in both captured shapes.

Queue fullness alone does not establish a performance bottleneck: naive M256 has {number(weight256['fetch_blocked_no_slot'])} no-slot fetch cycles but only {number(r['naive256']['counts']['compute_wait_weight'])} compute weight-wait cycles. Its writer head is deliberately held behind weight-consumption dependencies for {number(weight256['writer_head_blocked'])} cycles. At M4 that writer-head block is only {number(weight4['writer_head_blocked'])} cycles, and source-read completion is followed promptly by installation. The four-slot capacity interacts with the slow gather return path at M4; its independent performance cost still requires the proposed capacity experiment.
...
At M4, both DMA paths count 180224 read bytes and 4096 write bytes. External-DMA union-busy time is {number(mem['naive4']['external_union_busy'])} cycles in naive and {number(mem['improve4']['external_union_busy'])} in improve. The payload is the same, while improve transfers it over a shorter active interval.
...
Across the full invocation, the node blocks PSUM read issue for {number(r['naive4']['counts']['psum_rd_order_block'])} cycles at M4 and {number(r['naive256']['counts']['psum_rd_order_block'])} at M256. M4 mostly hides this behind its slower weight stream; its PSUM-only postprocessing block is only {number(r['naive4']['counts']['post_psum_only_block'])} cycles.

In the 4096-cycle window at configuration+50000..54095:

| Observation | Cycles or count |
|---|---:|
| Input accepted | {number(physical['input_fire'])} |
| External DMA busy / weight wait | 0 / 0 |
| PSUM prefetch allocations / late allocations | {number(physical['prefetch_allocations'])} / 0 |
| PSUM core request stalled / physical join full | 0 / 0 |
| Adapter slots used, maximum / capacity | 17 / 32 |
| Raw PSUM read blocked by node ordering | {number(physical['read_order_block'])} |
| Pending bank-set write conflict | {number(physical['read_pending_write_block'])} |
| Current bank-set write conflict | {number(physical['read_current_write_block'])} |
| Current conflicts with different complete read/write addresses | {number(physical['current_conflict_different_address'])} |
| Postprocessing head waiting for PSUM | {number(physical['post_wait_psum'])} |
| All-bank request utilization | {physical['bank_request_words']/65536:.2%} |
| Busiest individual bank utilization in this window | {max(physical['bank_accepted_by_index'])/4096:.2%} |
...
Full-invocation naive LMEM bank utilization is {mem['naive256']['bank_utilization']:.2%} at M256, consistent with the selected window. This is an aggregate metric; temporary hot-bank contention remains possible.
...
3. Neither the M4 all-bank utilization of {mem['naive4']['bank_utilization']:.2%} nor the measured M256 window demonstrates aggregate LMEM saturation. Average utilization also cannot rule out a temporary hot-bank bottleneck.
