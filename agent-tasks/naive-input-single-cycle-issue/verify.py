#!/usr/bin/env python3
"""Measure one frozen naive Input DMA issue candidate with VCS and FSDB."""
import argparse
import hashlib
...
def main():
    ...
    parser.add_argument('--config', type=Path, default=ROOT / 'configs/naive_gemm_th16_tcol16_hwexp_dcache_sxbar_f16.sh')
    ...
    build = ROOT / 'build_naive_input_issue_vcs'
    config = args.config.resolve()
    out = TASK / 'runs' / args.iteration / args.case
    if out.exists():
    ...
    if args.rebuild:
        cmd.append('--rebuild')
    print(f'Starting pipelined Input {args.case}: {out}', flush=True)
    ...
    wave = out / 'wave.fsdb'
    result = dict(pipelined_input_issue=True, read_quota=1, case=args.case, m=m, k=k, n=n,
