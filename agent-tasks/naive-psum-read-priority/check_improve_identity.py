...
OLD=TASK/'snapshot/rtl';NEW=ROOT/'hw/rtl';OUT=TASK/'runs/improve-identity';OUT.mkdir(parents=True,exist_ok=True)
flags=shlex.split(subprocess.check_output(['bash','-c','source "$1"; printf "%s" "$CONFIGS"','bash',str(TASK/'baseline-improve.sh')],cwd=ROOT,text=True))
