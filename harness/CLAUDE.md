...
- When starting a non-trivial task, create `docs/<task_name>/STATUS.yaml` and maintain it throughout the work. Log entries must include timestamps (`YYYY-MM-DD HH:MM` format, use `date` command) and cover: what was attempted, what failed and why, and what was learned. Additionally, maintain a **## Pitfalls** section at the bottom of STATUS.yaml to record failures, confusing behaviors, and gotchas encountered during the task. This section serves as a persistent reference so the same mistakes are not repeated.
