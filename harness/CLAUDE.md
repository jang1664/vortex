...
tools/hw_draw/                     # architecture design JSON + visualizer
third_party/                       # external dependencies (axi/)
ci/                                # CI scripts (run_black.sh, etc.)
build/                             # build output (gitignored)
```

## Branch Context — fpint_naive
- This branch uses the MMIO job descriptor frontend plus an internal GEMM FSM command generator.
- Default FPINT GEMM regression app: `tests/regression/fpint_gemm_ffn_hw`.
- Do not assume the `fpint_improve` instruction-stream frontend, `VX_cmd_constructor.sv`, or `tests/regression/fpint_gemm_ffn_hw_improve` flow unless the user explicitly asks for that variant.
- `gemm_unified_cmd_t` is an internal RTL payload produced by `VX_gemm_fsm.sv`; it is not a software-visible instruction-stream ABI in this branch.
...
- Always use `python3 tools/hw_draw/hw_tool.py --file tools/hw_draw/hw_arch.json ...` to read/modify hw_design_json files — do NOT edit JSON directly.
...
- FPINT GEMM naive RTL → @hw/rtl/core/gemm/, @hw/rtl/core/VX_job_frontend.sv, @harness/rules/rtl-arch.md
- FPINT GEMM naive software/regression → @tests/regression/fpint_gemm_ffn_hw/, @kernel/src/fi_gemm.c, @harness/rules/kernel-arch.md
- Runtime / XRT simulation → @runtime/xrt/, @sim/xrtsim/, @sim/xrtsim_vcs/, @hw/rtl/afu/xrt/
- Task FSMs → @agent-tasks/**/fsm.json and @agent-tasks/**/STATUS.yaml when those files exist
