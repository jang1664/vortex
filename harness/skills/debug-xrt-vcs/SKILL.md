...
```bash
cd /home/jaeyongjang/project.local/vortex_naive/build
PATH=/usr/bin:$PATH CONFIGS="$CONFIGS" timeout 300 \
  ./ci/blackbox.sh --driver=xrt_vcs --app=fpint_gemm_ffn_hw --args="<args>" --cores=1 --threads=8 --debug=3
```
...
For detailed address mapping mechanics, inspect the current code in `runtime/xrt/vortex_v*.cpp`, `sim/xrtsim*/`, and `hw/rtl/afu/xrt/`. This branch does not carry a dedicated HBM interleaving design note.
...
| File | Role |
|------|------|
| `runtime/xrt/vortex_v*.cpp` | XRT runtime variants (BO management, DMA, MMIO) |
