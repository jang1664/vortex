...
Run a regression test via `ci/run_black.sh` or `ci/blackbox.sh` from the build directory.
...
```
This generates runnable scripts from source-tree templates, including `ci/blackbox.sh` and wrapper scripts when present.

## Usage

All commands run from the `build/` directory. For `fpint_naive`, prefer the wrapper and canonical app:
...
If the wrapper is unavailable, call `ci/blackbox.sh` directly:
...
DEBUG_LEVEL=3
CONFIGS="$CONFIGS" \
./ci/blackbox.sh --driver=<driver> --app=fpint_gemm_ffn_hw --args="<args>" --cores=$NUM_CORES --threads=$NUM_THREADS --debug=$DEBUG_LEVEL
```
