...
# Finds the active task's STATUS.yaml and tells the agent to read it post-compaction.

PROJECT_DIR="${CLAUDE_PROJECT_DIR:-$(pwd)}"

# Find all STATUS.yaml files recursively (supports nested subtasks)
STATUS_FILES=$(find "$PROJECT_DIR/agent-tasks" -name "STATUS.yaml" -o -name "STATUS.md" 2>/dev/null | sort)
