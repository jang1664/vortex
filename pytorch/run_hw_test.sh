...
while IFS= read -r f; do
  ...
  ls test_native_*.py test_mm_w4a16_opt.py test_mm_w4a16_gemm_core_hw.py \
     ...
     test_spinquant_stack_accuracy_vortex_integration.py \
...
OP_FILE["spinquant_stack_accuracy_vortex_integration_full"]="test_spinquant_stack_accuracy_vortex_integration.py"
...
for op in "${OPS[@]}"; do
  ...
  if [[ "$op" == "spinquant_layer_accuracy_vortex_integration_full" ]]; then
    ...
    rc=$?
  elif [[ "$op" == "spinquant_stack_accuracy_vortex_integration_full" ]]; then
    RUN_SPINQUANT_STACK_FULL=1 SPINQUANT_STACK_LAYERS=32 python "${OP_FILE[$op]}"
    rc=$?
