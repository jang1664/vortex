#!/usr/bin/env python3
"""
Test: Native aten::add on Vortex device — CUDA-like flow.

This test demonstrates the full zero-copy-in-kernel flow:
  1. Create tensors on CPU
  2. .to('vortex')  →  one-time host→device copy
  3. a + b          →  eladd RISC-V kernel runs entirely on device memory
  4. .cpu()         →  one-time device→host copy
  5. Verify against CPU reference

No CPU fallback is used for the addition.  The RISC-V kernel binary
(eladd.vxbin) executes on the Vortex SimX simulator.
"""
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("  Vortex Native Add — CUDA-like Flow Test")
