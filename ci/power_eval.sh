...
FPGA_BIN_DIR=${FPGA_BIN_DIR:-/opt/vortex_fpga_bins/fpint/xrt_hw_u55c_c1_f100_fpint_tcu_noDcache_L2cache_a917286dbe/bin}
...
shape_args () {
  # Per-shape REPS sized so each blackbox.sh call runs ~10-20s of FPGA kernel
  # (kernel duty ≥ 80% over the run window).  -p enables power-mode in the app.
  local pflag=""
  [ "$POWER_MODE" = "1" ] && pflag=" -p"
  case "$1" in
    small)     echo "-m 512  -n 512  -k 512  -q 32 -t 0 -d 0 -r 800${pflag}" ;;   # ~23ms × 800 ≈ 18s
