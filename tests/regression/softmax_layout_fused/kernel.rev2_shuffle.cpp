...
static inline void softmax_tiled_shuffle(const TiledAccessor& accessor,
  ...
  for (; exp_k + NUM_THREADS < k_end; exp_k += 2u * NUM_THREADS) {
    const float exp0 = vx_expf(scores[exp_k] - global_max);
    const float exp1 = vx_expf(scores[exp_k + NUM_THREADS] - global_max);
    scores[exp_k] = exp0;
  ...
  if (exp_k < k_end) {
    const float exp_value = vx_expf(scores[exp_k] - global_max);
  ...
  for (uint32_t k = lane; k < k_end;
    ...
    accessor.output[accessor.output_row_prefix + within_matrix] =
        float_to_fp16(scores[k] * inv_sum);
  }
  ...
  if (store_k < k_end) {
    ...
    for (; store_k + NUM_THREADS < k_end; store_k += 2u * NUM_THREADS) {
      *store0 = float_to_fp16(scores[store_k] * inv_sum);
      *store1 = float_to_fp16(scores[store_k + NUM_THREADS] * inv_sum);
