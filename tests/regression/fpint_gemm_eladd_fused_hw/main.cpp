...
static uint32_t SCHEDULE = SCHEDULE_FUSED;
static bool REQUIRE_OVERLAP = false;
...
static void show_usage() {
  ...
  std::cout << "       [-r REPS] [-s fused|sequential]" << std::endl;
  std::cout << "       [--require-overlap]" << std::endl;
  std::cout << "       [-h]" << std::endl;
}

static struct option long_options[] = {
  {"schedule", required_argument, nullptr, 's'},
  {"require-overlap", no_argument, nullptr, 'o'},
  {nullptr, 0, nullptr, 0},
};
...
static void parse_args(int argc, char **argv) {
  int c;
  int option_index = 0;
  while ((c = getopt_long(argc, argv, "m:n:k:q:t:d:r:s:oh", long_options, &option_index)) != -1) {
    switch (c) {
    ...
    case 's':
      if (strcmp(optarg, "fused") == 0) SCHEDULE = SCHEDULE_FUSED;
      else if (strcmp(optarg, "sequential") == 0) SCHEDULE = SCHEDULE_SEQUENTIAL;
      else { show_usage(); exit(-1); }
      break;
    case 'o': REQUIRE_OVERLAP = true; break;
...
static int verify_results_row_major(vx_buffer_h out_buffer,
                                    const std::vector<uint16_t>& ref) {
  std::vector<uint16_t> actual(ref.size());
  RT_CHECK(vx_copy_from_dev(actual.data(), out_buffer, 0,
                            actual.size() * sizeof(uint16_t)));
  int errors = 0;
  for (size_t i = 0; i < ref.size(); ++i) {
    if (!compare_fp16(actual[i], ref[i], FP16_TOL)) {
      if (errors < 10) {
        const uint32_t m = uint32_t(i / N);
        const uint32_t n = uint32_t(i % N);
        printf("Mismatch[m=%u,n=%u]: got=0x%04x (%f), exp=0x%04x (%f)\n",
               m, n, unsigned(actual[i]), fp16_to_float(actual[i]),
...
int main(int argc, char *argv[]) {
  ...
  if (QBLK == 0 || (QBLK & (QBLK - 1)) != 0 || WTRANS > 1 || QDIR > 1) {
  ...
  std::cout << "M=" << M << " (padded to " << M_pad << ")"
            ...
            << ", QDIR=" << QDIR << ", schedule="
            << (SCHEDULE == SCHEDULE_FUSED ? "fused" : "sequential") << std::endl;
  ...
  std::vector<uint16_t> h_fused_ref(M * N);
  for (size_t i = 0; i < h_residual.size(); ++i) {
    const float residual = float(int(i % 17) - 8) / 16.0f;
    h_residual[i] = float_to_fp16(residual);
    h_fused_ref[i] = float_to_fp16(fp16_to_float(h_ref_out_fp16[i])
                                  + fp16_to_float(h_residual[i]));
  }
  ...
  std::vector<uint16_t> zero_fused(h_fused_ref.size(), 0);
  RT_CHECK(vx_copy_to_dev(fused_output_buffer, zero_fused.data(), 0,
                          zero_fused.size() * sizeof(uint16_t)));
  ...
  for (uint32_t rep = 0; rep < REPS; ++rep) {
    ...
    for (uint32_t core = 0; core < num_cores; ++core) {
      if (kargs.core_status[core] != STATUS_OK) {
        std::cout << "Kernel failed on core " << core
                  << ": status=" << kargs.core_status[core] << std::endl;
        cleanup();
        return -1;
      }
    }
  ...
  if (SCHEDULE == SCHEDULE_FUSED) {
    ...
    for (uint32_t core = 0; core < active_cores; ++core) {
      std::cout << "Overlap core " << core << ": "
                ...
                << std::endl;
      if (REQUIRE_OVERLAP && kargs.overlap_observed[core] == 0) {
        std::cout << "FAILED: no GEMM/ELADD overlap observed on core " << core
                  << std::endl;
        ++errors;
      }
    }
