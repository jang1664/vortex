...
// Destination layout (row-major, real M rows only):
//   dst[ m * N + (nt*MXU_NT + n_in_sub) ]
///////////////////////////////////////////////////////////////////////////////
...
void kernel_detile_output(kernel_arg_t *__UNIFORM__ arg) {
  ...
  constexpr uint32_t MXU_NT = TILE_DMA_MXU_NT;
  ...
  // Source: nt-major
  const uint32_t per_nt_elems = M_pad * MXU_NT;
  const uint64_t src_byte_off =
      ((uint64_t)nt * per_nt_elems + (uint64_t)m * MXU_NT + ncol) * TILE_ELEM_BYTES;

  // Destination: row-major [M, N]
  const uint64_t dst_byte_off =
      ((uint64_t)m * N + (uint64_t)nt * MXU_NT + ncol) * TILE_ELEM_BYTES;
