...
static constexpr uint32_t kPairsPerTask = 16u;
static constexpr uint32_t kTasksPerWarp = NUM_THREADS / kPairsPerTask;
...
void kernel_rope_layout_fused(kernel_arg_t *__UNIFORM__ arg) {
  ...
  const uint32_t chunks = half_dim >> 4;
  ...
  const uint32_t p_in_chunk = lane & 15u;
  const uint64_t q_matrix_elems =
      (uint64_t)arg->output_m_pad * head_dim;
  const uint64_t k_matrix_elems =
      (uint64_t)head_dim * arg->max_seq_len;

  // Two 16-lane subwarps each own one rotary chunk. Lanes within a subgroup
  // read adjacent tiled elements and write adjacent output elements.
  for (uint32_t task = warp_id * kTasksPerWarp + subgroup;
    ...
    const uint32_t p = (chunk << 4) + p_in_chunk;
    ...
    const uint64_t x1_off = gemm_c_tiled_elem_offset(
        ...
        arg->log2_mt, arg->log2_mxu_nt);
    const float c =
        fp16_to_float(cos_table[(uint64_t)pos * half_dim + p]);
    const float sn =
        fp16_to_float(sin_table[(uint64_t)pos * half_dim + p]);
    const float x0 = fp16_to_float(input[x0_off]);
    const float x1 = fp16_to_float(input[x1_off]);
    const data_t y0 = float_to_fp16(x0 * c - x1 * sn);
    const data_t y1 = float_to_fp16(x0 * sn + x1 * c);

    if (arg->layout_to == ROPE_LAYOUT_TO_GEMM_A) {
      const uint64_t out_base =
          batched_matrix_base(matrix_idx, q_matrix_elems);
      const uint64_t y0_off = out_base + gemm_a_tiled_elem_offset(
          s, p, arg->output_m_pad, head_dim,
      ...
      const uint64_t out_base =
          batched_matrix_base(matrix_idx, k_matrix_elems);
      ...
      const uint64_t out_base =
          (arg->layout_to == ROPE_LAYOUT_TO_HEAD_MAJOR_ROW)
          ? (((uint64_t)b * heads + h) * seq_len + s) * head_dim
          : (((uint64_t)b * seq_len + s) * heads + h) * head_dim;
      output[out_base + p] = y0;
      output[out_base + p + half_dim] = y1;
    }
  }
}
