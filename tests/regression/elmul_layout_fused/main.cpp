...
int main(int argc, char *argv[]) {
  ...
  if (K % TILE_DMA_MXU_KT != 0 || K % TILE_DMA_MXU_NT != 0) {
    printf("ERROR: K must be a multiple of %u and %u\n", TILE_DMA_MXU_KT, TILE_DMA_MXU_NT);
    return 1;
  }
  ...
  for (uint32_t m = 0; m < M; ++m) {
    for (uint32_t k = 0; k < K; ++k) {
      const uint64_t off = gemm_a_tiled_elem_offset(m, k, M_pad, K, log2_mt, log2_mxu_kt);
      const float expected = fp16_to_float(float_to_fp16(
          fp16_to_float(h_a_row[(uint64_t)m * K + k]) *
          fp16_to_float(h_b_row[(uint64_t)m * K + k])));
