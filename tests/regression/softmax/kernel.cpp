...
void kernel_softmax(kernel_arg_t *__UNIFORM__ arg) {
  ...
  if (active) {
    for (uint32_t k = tid; k < seq_len_k; k += block_size) {
      ...
      // Store intermediate exp values (we'll use them again)
      output_row[k] = exp_val;
      local_sum += exp_val;
    }
  }
  ...
  // Step 3: Normalize
  //===========================================================================
  // Only compute if active (avoid division by zero for inactive blocks)
  float inv_sum = active ? (1.0f / global_sum) : 0.0f;
  
  if (active) {
    for (uint32_t k = tid; k < seq_len_k; k += block_size) {
      output_row[k] *= inv_sum;
    }
  }
}
