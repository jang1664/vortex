// Benchmark harness for eldiv. See softmax/bench_main.cpp for design notes.
...
static void initialize_random(std::vector<data_t>& vec, bool avoid_zero) {
  for (auto& v : vec) {
    ...
    if (avoid_zero && std::abs(v) < 0.1f) v = (v >= 0) ? 0.1f : -0.1f;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (!bench.csv) {
    printf("Eldiv Bench: size=%u  warmup=%d iterations=%d\n",
           size, bench.warmup, bench.iterations);
  }

  std::vector<data_t> h_a, h_b;
  ...
  if (!vx_bench::run_power_measurement(
          "eldiv", bench, device, krnl_buffer, args_buffer, bench.power_measure_latency)) {
    cleanup();
    return -1;
  }
