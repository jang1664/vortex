...
// Helper functions
///////////////////////////////////////////////////////////////////////////////
void initialize_random(std::vector<data_t>& vec, bool avoid_zero = false) {
  for (auto& val : vec) {
    val = static_cast<float>(rand()) / RAND_MAX * 4.0f - 2.0f;  // [-2, 2]
    if (avoid_zero && std::abs(val) < 0.1f) {
      val = (val >= 0) ? 0.1f : -0.1f;  // Avoid division by zero
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  printf("Element-wise Divide Test Configuration:\n");
  ...
  // Initialize data
  srand(42);
  initialize_random(h_input_a, false);
  initialize_random(h_input_b, true);  // Avoid zeros in divisor
  
  // Run CPU reference
  printf("Running CPU reference...\n");
  eldiv_cpu(h_input_a, h_input_b, h_output_cpu);
  ...
  kernel_arg.kernel_id = KERNEL_ELDIV;
  ...
  for (uint32_t i = 0; i < size; ++i) {
    ...
    // Use relative error for division
