...
`endif
  ...
  task automatic wait_job_done(input int unsigned eid);
    ...
    do begin
      ...
      @(posedge clk);
      timeout++;
      if (timeout > 100000) $fatal(1, "[%0t] wait_job_done timeout (ctrl=0x%08h)", $time, ctrl);
