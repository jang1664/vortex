...
module tb_mem_bus_split_depth import VX_gpu_pkg::*; #(
    ...
    REORDER=0, INJECT_REORDER=0, TAG_BITS=12
) ();
    localparam TAGW=`UP(UUID_WIDTH)+TAG_BITS;
    ...
    VX_mem_bus_split #(.NUM_LANES(LANES),.LANE_DATA_SIZE(BYTES),
        .TAG_WIDTH(TAGW),.ENABLE_LANE_MASK(MASKED),.RSP_DEPTH(DEPTH),.RSP_REORDER(REORDER)) dut(
    ...
    always @(negedge clk) begin
        ...
        wide.req_data.tag.uuid=`UP(UUID_WIDTH)'(sent>>2);
    ...
    for(genvar l=0;l<LANES;l++) begin:g_model
        ...
        initial begin
            ...
            lane_accepted[l]=0;reordered[l]=0;
        end
        always @(negedge clk) begin
            ...
            if(!holding) begin
                ...
                if(!reset && pending.size()>0
                    ...
                    selection=(INJECT_REORDER && pending.size()>1 && (cycle+l+SEED)%3!=0)?1:0;
    ...
    always @(posedge clk) if(!reset) begin
        ...
        if(wide.rsp_valid && wide.rsp_ready) begin
            ...
            assert(wide.rsp_data.tag.value==TAG_BITS'(id)) else $fatal(1,"response tag mismatch");
            assert(wide.rsp_data.tag.uuid==`UP(UUID_WIDTH)'(id>>2)) else $fatal(1,"response UUID mismatch");
            for(int l=0;l<LANES;l++) begin
        ...
        if(sent==COUNT && received==expected_reads) begin
            int reorder_count;
            reorder_count=0;
            for(int l=0;l<LANES;l++)reorder_count+=reordered[l];
            assert(!INJECT_REORDER || reorder_count>0) else $fatal(1,"no out-of-order response injected");
            for(int l=0;l<LANES;l++) begin
                int n;
                n=0;
            ...
            $display("TEST PASSED: lanes=%0d bytes=%0d depth=%0d masked=%0d seed=%0d reads=%0d cycles=%0d continuous=%0d reordered=%0d",LANES,BYTES,DEPTH,MASKED,SEED,received,cycle,max_consecutive,reorder_count);
            $finish;
        end
