...
module tb_VX_dma_max_dims_invalid import VX_gpu_pkg::*; #(
  parameter int INVALID_MAX_DIMS = 0
) ();
  ...
  VX_config_reg_if #(.NUM(CFG_NUM), .DW(32)) cfg_if ();
  ...
  VX_dma_unit_align #(
    .INSTANCE_ID("max-dims-invalid"),
    .MAX_DIMS(INVALID_MAX_DIMS),
    .DCACHE_ADDR_WIDTH(MEM_ADDR_WIDTH - `CLOG2(DATA_SIZE)),
    .LMEM_ADDR_WIDTH(MEM_ADDR_WIDTH - `CLOG2(DATA_SIZE)),
    .DCACHE_TAG_WIDTH(TAG_WIDTH),
    .LMEM_TAG_WIDTH(TAG_WIDTH),
    .RD_OUTSTANDING(4)
  ) dut (
    .clk(clk), .reset(reset), .cfg_reg_if(cfg_if),
    .lookahead_if(lookahead_if), .dcache_bus_if(dcache_if),
    .lmem_bus_if(lmem_if), .done_if(done_if)
  );

  initial begin
    #100;
    $fatal(1, "invalid MAX_DIMS guard did not fire");
  end
