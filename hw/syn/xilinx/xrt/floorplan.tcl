...
proc ::vortex::slr::enabled {} {
    if {[info exists ::env(VORTEX_DMA_CHANNEL_FLOORPLAN)] && $::env(VORTEX_DMA_CHANNEL_FLOORPLAN) ne "0"} {
        error "DMA_CHANNEL_FLOORPLAN is retired; narrow DMA pblocks are forbidden"
    }
    set value 0
...
proc ::vortex::slr::owner_for {path} {
    ...
    if {[string match {u_tmem_subsystem/*} $path]} {
        ...
        error "unclassified TMEM subsystem leaf: $path"
    }
...
proc ::vortex::slr::matching {cells expression} {
    set result {}
    foreach cell $cells {if {[regexp $expression $cell]} {lappend result $cell}}
    return $result
}
proc ::vortex::slr::inventory {} {
    ...
    if {![info exists ::env(VORTEX_GEMM_TMEM_BANKS)] || $::env(VORTEX_GEMM_TMEM_BANKS) ni {8 16}} {
        error "SLR floorplan requires source VORTEX_GEMM_TMEM_BANKS=8 or 16"
    }
    set arrays $::env(VORTEX_GEMM_TMEM_BANKS)
    if {![info exists ::env(VORTEX_GEMM_MXU_COL)]
        || $::env(VORTEX_GEMM_MXU_COL) ne [expr {$arrays == 8 ? 32 : 16}]} {
        error "SLR floorplan MXU geometry does not match the selected TMEM profile"
    }
    foreach root $roots {
        set local {}
        ...
        set found_arrays [dict create]; set found_channels [dict create]; set found_pairs [dict create]
        foreach leaf $local {
        ...
        if {[lsort -integer [dict keys $found_arrays]] ne $expected_arrays || [lsort -integer [dict keys $found_channels]] ne {0 1 2 3 4 5 6 7}} {
            error "$root: unexpected physical TMEM array or HBM DMA channel indices"
        }
        if {[dict size $found_pairs] != ($arrays == 16 ? 8 : 0)} {error "$root: incorrect pair-adapter count"}
        # g_direct is wires only and may disappear. Its required signature is
        # eight arrays/channels and zero pair adapters, not an empty hierarchy.
