...
// Vortex-local copy based on CVFPU commit a6af691.
// Vortex permits scalar FP/INT conversions across widths, so a CONV lane must
// accommodate enabled INT64 formats even when the FP64 format is disabled.
...
FP8. Please use the PULP DivSqrt unit when in need of div/sqrt operations on FP8.");
  ...
  for (genvar lane = 0; lane < int'(NUM_LANES); lane++) begin : gen_num_lanes
    ...
    // Scalar casts may target an integer format wider than every enabled FP format.
    localparam int unsigned CONV_INT_WIDTH = fpnew_pkg::max_int_width(CONV_INT_FORMATS);

    // Lane parameters from Opgroup
    localparam fpnew_pkg::fmt_logic_t LANE_FORMATS = (OpGroup == fpnew_pkg::CONV)
                                                     ? CONV_FORMATS : ACTIVE_FORMATS;
    localparam int unsigned LANE_WIDTH = (OpGroup == fpnew_pkg::CONV)
                                         ? fpnew_pkg::maximum(CONV_WIDTH, CONV_INT_WIDTH)
                                         : MAX_WIDTH;
