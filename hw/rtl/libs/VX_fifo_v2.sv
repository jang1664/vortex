...
module VX_fifo_v2 #(
    ...
    input  logic  rst_ni,           // Asynchronous rstn active low
  ...
  always_comb begin : read_write_comb
    ...
    // allow push when not full, OR when full but pop is also happening
    if (push_i && (~full_o || (full_o && pop_i))) begin
...
`ifndef VERILATOR
  ...
  full_write : assert property(
      @(posedge clk_i) disable iff (~rst_ni) (full_o |-> (~push_i || pop_i)))
  else $fatal (1,"Trying to push new data although the FIFO is full without pop.");
