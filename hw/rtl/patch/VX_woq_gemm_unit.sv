...
// VX_woq_gemm_unit — Weight-only-quant variant of VX_gemm_unit.
// Removed vs the WKV original (which is in VX_gemm_unit.sv):
//   1) gen_in_scaler block + related signals/pipes (no per-row activation
//      pre-scale; activation goes straight into the prealigner).
//   2) The QROW data path in `gen_pre_proc_route` (act_reduce ↔ zp_mul
//      feedback loop). Only the QCOL path is kept; `is_qcol`/`is_qrow`
//      mux is removed.
//   3) MXU column-direction weight load — instantiates `VX_woq_gemm_tree`
//      (and its `VX_woq_gemm_weight_regs_v1`/`VX_woq_pe_tree`) which has
//      `weight_load_dir_i` and the col-shift mux removed.
// Same vec-op module wrap (VX_vec_*) as the WKV variant for direct
// hierarchy-by-hierarchy comparison.

module VX_woq_gemm_unit import VX_gpu_pkg::*; #(
    ...
    // -------------------------------------------------------------------------
    // PATCH (component_database): externalized accumulator memory interface.
    //   Original `gen_acc_mem` instantiated 4x VX_sp_ram (OUT_REG=1, depth =
    //   GEMM_ACC_MEM_DEPTH). Storage is removed for ASIC compute-only synth;
    //   external memory model must provide 1-cycle read latency to preserve
    //   pipeline timing. FP32_WIDTH localparam is not yet in scope at the
    //   port-list, so the data width is spelled with literal 32.
    // -------------------------------------------------------------------------
    output logic [3:0]                                          acc_mem_wr_en_o,
    output logic [3:0]                                          acc_mem_rd_en_o,
    output logic [3:0][`GEMM_ACC_MEM_BANK_DEPTH_ADDR_WIDTH-1:0] acc_mem_wr_addr_o,
    output logic [3:0][`GEMM_ACC_MEM_BANK_DEPTH_ADDR_WIDTH-1:0] acc_mem_rd_addr_o,
    output logic [3:0][`MXU_COL-1:0][31:0]                      acc_mem_wdata_o,
    input  logic [3:0][`MXU_COL-1:0][31:0]                      acc_mem_rdata_i
`ifdef PERF_ENABLE
...
`endif
    ...
    // PATCH (WoQ): no in_scaler — prealigner consumes in_pipe directly.
    assign prealigner_in_data  = in_pipe_data_out;
    assign prealigner_in_valid = in_pipe_valid_out;
    ...
    // -------------------------------------------------------------------------
    // PATCH (WoQ): in_scaler block (per-row activation × scale) is removed
    // entirely. Activation flows in_pipe_data_out → prealigner_in_data
    // directly (see assign above). The in_scaler_*  signals declared above
    // are unused in WoQ and trimmed by DC; we keep the declarations so the
    // signal-list section stays diff-friendly with the WKV variant.
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // PATCH (WoQ): QCOL-only data path. The is_qcol mux + zp_mul→act_reduce
    // feedback loop are removed. Activation flow is straight:
    //   prealigner_int_data → act_reduce_in → act_reduce_out → zp_mul_in
    generate
        for (genvar i = 0; i < `MXU_ROW; i++) begin : gen_pre_proc_route
            assign act_reduce_data_in[i] = signed'(prealigner_int_data[i]);
            assign act_reduce_blk_idx[i] = prealigner_blk_idx[i];
            if(i == 0) begin
              assign act_reduce_valid_in = prealigner_out_valid;
            end
            assign zp_mul_in_data[i] = signed'(act_reduce_data_out);
            if(i == 0) begin
              assign zp_mul_in_valid = act_reduce_valid_out;
            end
        end
    endgenerate
    ...
    VX_vec_signed_shl #(
        ...
        .OUT_W      (`ACT_REDUCE_IN_WIDTH),
        .SHAMT_W    (`BLOCK_IDX_WIDTH),
        .BLOCK_SIZE (`BLOCK_SIZE)
    ) u_act_reduce_shl_vec (
        .in_i    (act_reduce_data_in),
        .shamt_i (act_reduce_blk_idx),
        .out_o   (act_reduce_data_in_shifted)
    );
    ...
    generate
        for (genvar i = 0; i < `MXU_COL; i++) begin : gen_pre_proc_out
            assign pre_proc_out[i]    = signed'(zp_mul_out_data_q[i]);
            if(i == 0) begin
              assign pre_proc_in_valid  = zp_mul_out_valid;
            end
        end
    endgenerate
    ...
    // -------------------------------------------------------------------------
    // GEMM Tree (MXU) — PATCH (WoQ): VX_woq_gemm_tree removes the
    // column-direction weight-load path (and `weight_load_dir_i` port).
    // -------------------------------------------------------------------------
    VX_woq_gemm_tree u_mxu (
        .clk_i            (clk),
        .resetn_i         (~reset),
    ...
    // -------------------------------------------------------------------------
    // Merger (PATCH: wrapped as VX_vec_signed_add for hierarchy breakdown)
    // -------------------------------------------------------------------------
    VX_vec_signed_add #(
        .N    (`MXU_COL),
        .A_W  (`O_BIT_WIDTH),
    ...
    // -------------------------------------------------------------------------
    // Integer to FP Converters (PATCH: wrapped as VX_vec_pint2fp)
    // -------------------------------------------------------------------------
    VX_vec_pint2fp #(
        .N                 (`MXU_COL),
        .IN_DW             (`MERGE_OUT_BW),
        ...
        .clk_i      (clk),
        .resetn_i   (~reset),
        .int_data_i (merger_out_data_q),
        .max_exp_i  (prealigner_max_exp_q),
        .valid_i    ({`MXU_COL{merger_out_valid}}),
        .fp_data_o  (int2fp_out_data),
        .valid_o    (int2fp_output_valid)
    );
    ...
    // PATCH: per-element wiring kept inline (just mux/format), but actual
    // multipliers wrapped as VX_vec_fp16_mul_outscale or VX_vec_fp32_mul.
`ifdef GEMM_UNIT_FP16_OUT_SCALE
    logic [`MXU_COL-1:0]                    out_scaler_a_valid_vec, out_scaler_b_valid_vec;
    logic [`MXU_COL-1:0][FP16_WIDTH-1:0]    out_scaler_a_data_vec,  out_scaler_b_data_vec;
`else
    logic [`MXU_COL-1:0]                    out_scaler_a_valid_vec, out_scaler_b_valid_vec;
    logic [`MXU_COL-1:0][FP32_WIDTH-1:0]    out_scaler_a_data_vec,  out_scaler_b_data_vec;
`endif

    // PATCH (WoQ): is_qcol gating dropped — out scaler is always active.
    generate
        for (genvar i = 0; i < `MXU_COL; i++) begin : gen_out_scaler_drv
            assign out_scaler_a_valid_vec[i] = int2fp_output_valid[i];
            assign out_scaler_b_valid_vec[i] = int2fp_output_valid[i];
            assign out_scaler_a_data_vec[i]  = int2fp_output_valid[i] ? int2fp_out_data[i] : '0;
`ifdef GEMM_UNIT_FP16_OUT_SCALE
            assign out_scaler_b_data_vec[i]  = int2fp_output_valid[i]
                ? scale_regs[gemm_unit_ctrl.sreg_use_idx][i] : '0;
`else
            logic [FP16_EXP_WIDTH-1:0] exp_local;
            assign exp_local = scale_regs[gemm_unit_ctrl.sreg_use_idx][i][14:10];
            assign out_scaler_b_data_vec[i][31]    = int2fp_output_valid[i]
                ? scale_regs[gemm_unit_ctrl.sreg_use_idx][i][15] : '0;
            assign out_scaler_b_data_vec[i][30:23] = int2fp_output_valid[i]
                ? ((&exp_local==1'b1) ? '1 : exp_local + (FP32_EXP_BIAS - FP16_EXP_BIAS)) : '0;
            assign out_scaler_b_data_vec[i][22:0]  = int2fp_output_valid[i]
                ? {scale_regs[gemm_unit_ctrl.sreg_use_idx][i][9:0], 13'b0} : '0;
`endif
        end
    endgenerate
...
`ifdef GEMM_UNIT_FP16_OUT_SCALE
    VX_vec_fp16_mul_outscale #(
        ...
        .clk          (clk),
        .reset        (reset),
        .a_valid      (out_scaler_a_valid_vec),
        .a_ready      (),
        .a_data       (out_scaler_a_data_vec),
        .b_valid      (out_scaler_b_valid_vec),
        .b_ready      (),
        .b_data       (out_scaler_b_data_vec),
        .result_valid (scaler_output_valid),
        .result_ready ({`MXU_COL{1'b1}}),
        .result_data  (scaled_fp_out_data)
    );
`else
    VX_vec_fp32_mul #(
        .N       (`MXU_COL),
...
`endif
    ...
    // PATCH (WoQ): QROW bypass pipe + final mux removed — output scaler is
    // always taken (QCOL is the only mode in WoQ).
    assign final_scaled_fp_out_data  = scaled_fp_out_data;
    assign final_scaler_output_valid = scaler_output_valid[0];
    generate
...
`endif
    ...
    // -------------------------------------------------------------------------
    // Accumulators (PATCH: wrapped as VX_vec_fp32_add for hierarchy breakdown)
    // -------------------------------------------------------------------------
    logic [`MXU_COL-1:0][FP32_WIDTH-1:0] acc_a_data_vec, acc_b_data_vec;
    ...
    VX_vec_fp32_add #(
        ...
        .clk          (clk),
        .reset        (reset),
        .a_valid      (acc_in_data_valid),
        .a_ready      (),
        .a_data       (acc_a_data_vec),
        .b_valid      (acc_psum_data_valid),
        .b_ready      (),
        .b_data       (acc_b_data_vec),
        .result_valid (acc_output_valid),
        .result_ready ({`MXU_COL{1'b1}}),
        .result_data  (acc_output_data)
    );
...
`endif
    ...
    generate
        for (genvar i = 0; i < 4; i++) begin : gen_acc_mem
            ...
            // -----------------------------------------------------------------
            // PATCH (component_database): VX_sp_ram instance removed. Storage
            // is exposed via top-level ports `acc_mem_*_o` / `acc_mem_rdata_i`.
            // Caller is responsible for providing a memory model with OUT_REG=1
            // semantics (1-cycle read latency) to preserve pipeline timing.
            // -----------------------------------------------------------------
            assign acc_mem_wr_en_o[i]   = acc_mem_wr_en[i];
            assign acc_mem_rd_en_o[i]   = acc_mem_rd_en[i];
            assign acc_mem_wr_addr_o[i] = acc_mem_wr_depth_addr[i];
            assign acc_mem_rd_addr_o[i] = acc_mem_rd_depth_addr[i];
            assign acc_mem_wdata_o[i]   = acc_mem_in_data[i];
            assign acc_mem_out_data[i]  = acc_mem_rdata_i[i];
        end
    endgenerate
    ...
    // -------------------------------------------------------------------------
    logic [`MXU_COL-1:0][FP32_WIDTH-1:0] f32_to_f16_data_vec;
    generate
        for (genvar i = 0; i < `MXU_COL; i++) begin : gen_fp32_to_fp16_drv
            assign f32_to_f16_data_vec[i] = acc_mem_out_data[acc_mem_out_rd_bank_q][i];
        end
    endgenerate
    ...
    VX_vec_f32_to_f16 #(
        ...
        .clk_i    (clk),
        .resetn_i (~reset),
        .valid_i  ({`MXU_COL{acc_mem_rd_out_valid}}),
        .data_i   (f32_to_f16_data_vec),
        .valid_o  (fp16_out_valid),
        .data_o   (fp16_out_data)
    );
