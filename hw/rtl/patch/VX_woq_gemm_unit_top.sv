...
// Verilog wrapper for VX_woq_gemm_unit to enable synthesis without SystemVerilog interfaces

module VX_woq_gemm_unit_top import VX_gpu_pkg::*; #(
    ...
    // Externalized accumulator memory port widths — match the patched
    // VX_woq_gemm_unit.sv (acc_mem removed, signals exposed at port).
    parameter ACC_ADDR_W   = `GEMM_ACC_MEM_BANK_DEPTH_ADDR_WIDTH,
    parameter ACC_DATA_W   = `MXU_COL * 32
) (
    // Clock and Reset
    input wire                              clk,
    input wire                              reset,
    ...
    // =========================================================================
    // Externalized Accumulator Memory Bus (4 banks, see patch notes)
    // =========================================================================
    output wire [3:0]                                   acc_mem_wr_en_o,
    output wire [3:0]                                   acc_mem_rd_en_o,
    output wire [3:0][ACC_ADDR_W-1:0]                   acc_mem_wr_addr_o,
    output wire [3:0][ACC_ADDR_W-1:0]                   acc_mem_rd_addr_o,
    output wire [3:0][ACC_DATA_W-1:0]                   acc_mem_wdata_o,
    input  wire [3:0][ACC_DATA_W-1:0]                   acc_mem_rdata_i
);
