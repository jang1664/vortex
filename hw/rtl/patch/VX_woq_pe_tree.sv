...
/*
  PE Tree for WoQ GEMM (renamed copy of VX_pe_tree_new).
  - Functionally identical to VX_pe_tree_new (the original PE has no
    column-direction-load logic — that lives in the weight register file).
  - Renamed so the hierarchy power/area report shows it as a distinct
    instance under VX_woq_gemm_tree, separate from VX_pe_tree_new in the
    WKV variant.
*/

module VX_woq_pe_tree import VX_gpu_pkg::*, VX_utils_pkg::*; #(
