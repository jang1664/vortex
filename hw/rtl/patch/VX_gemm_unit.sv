...
`endif
    ...
    // Input Scalers
    // -------------------------------------------------------------------------
    // PATCH (component_database): replaced inline generate of VX_fp16_mul
    // instances with a single VX_vec_fp16_mul module so the in-scaler power
    // and area shows up as one named block in the hierarchy report.
    logic                                in_scaler_activated;
    logic [`MXU_ROW-1:0]                 in_scaler_a_valid_vec, in_scaler_b_valid_vec;
    logic [`MXU_ROW-1:0][`IFP_WIDTH-1:0] in_scaler_a_data_vec,  in_scaler_b_data_vec;

    assign in_scaler_activated = (gemm_unit_ctrl.quant_dir == `QDIR_ROW) & in_flight;
    generate
        for (genvar i = 0; i < `MXU_ROW; i++) begin : gen_in_scaler_drv
            assign in_scaler_a_valid_vec[i] = in_pipe_valid_out & in_scaler_activated;
            assign in_scaler_b_valid_vec[i] = in_scaler_a_valid_vec[i];
            assign in_scaler_a_data_vec[i]  = in_scaler_activated
                ? in_pipe_data_out[`IFP_WIDTH*i +: `IFP_WIDTH] : '0;
            assign in_scaler_b_data_vec[i]  = in_scaler_activated
    ...
    VX_vec_fp16_mul #(
        ...
        .clk          (clk),
        .reset        (reset),
        .a_valid      (in_scaler_a_valid_vec),
        .a_ready      (in_scaler_a_ready),
        .a_data       (in_scaler_a_data_vec),
        .b_valid      (in_scaler_b_valid_vec),
        .b_ready      (in_scaler_b_ready),
        .b_data       (in_scaler_b_data_vec),
        .result_valid (in_scaler_result_valid),
        .result_ready ({`MXU_ROW{1'b1}}),
        .result_data  (in_scaler_result_data)
    );
    ...
    // -------------------------------------------------------------------------
    // PATCH (component_database): keep mux/route wiring inline (small),
    // but pull the per-row variable shift out into a named module so the
    // act_reduce-input shifter shows up by itself in the hierarchy report.
    generate
...
`endif
    ...
    generate
        for (genvar i = 0; i < `MXU_COL; i++) begin : gen_out_scaler_drv
            assign out_scaler_a_valid_vec[i] = int2fp_output_valid[i] & is_qcol;
            assign out_scaler_b_valid_vec[i] = int2fp_output_valid[i] & is_qcol;
            assign out_scaler_a_data_vec[i]  = (int2fp_output_valid[i] & is_qcol) ? int2fp_out_data[i] : '0;
`ifdef GEMM_UNIT_FP16_OUT_SCALE
            assign out_scaler_b_data_vec[i]  = (int2fp_output_valid[i] & is_qcol)
                ? scale_regs[gemm_unit_ctrl.sreg_use_idx][i] : '0;
`else
            logic [FP16_EXP_WIDTH-1:0] exp_local;
            assign exp_local = scale_regs[gemm_unit_ctrl.sreg_use_idx][i][14:10];
            assign out_scaler_b_data_vec[i][31]    = (int2fp_output_valid[i] & is_qcol)
                ? scale_regs[gemm_unit_ctrl.sreg_use_idx][i][15] : '0;
            assign out_scaler_b_data_vec[i][30:23] = (int2fp_output_valid[i] & is_qcol)
                ? ((&exp_local==1'b1) ? '1 : exp_local + (FP32_EXP_BIAS - FP16_EXP_BIAS)) : '0;
            assign out_scaler_b_data_vec[i][22:0]  = (int2fp_output_valid[i] & is_qcol)
