...
// PATCH (component_database): the string-returning helpers below are used
// only inside `ifdef DBG_TRACE_GEMM `$display` calls. DC's synthesis
// elaborator rejects the SystemVerilog `string` type (VER-700), so we hide
// these functions from the synthesis tool while leaving them available for
// simulation. Callsites are all guarded by DBG_TRACE_*, so the missing
// symbol is harmless under synthesis.
`ifndef SYNTHESIS
