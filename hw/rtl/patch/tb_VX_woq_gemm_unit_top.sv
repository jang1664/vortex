// Steady-input testbench for VX_woq_gemm_unit_top.
...
//
// The DUT instance name `u_VX_woq_gemm_unit_top` and tb name `tb_VX_woq_gemm_unit_top`
// match hwexplorer's PwrConfig defaults so STRIP_PATH lines up automatically.

`timescale 1ns / 1ps
`include "VX_define.vh"

module tb_VX_woq_gemm_unit_top;
    ...
    // DUT
    // ----------------------------------------------------------------------
    VX_woq_gemm_unit_top u_VX_woq_gemm_unit_top (
        .clk            (clk),
        .reset          (reset),
    ...
    initial begin
        ...
        $fsdbDumpfile("./reports/tb_VX_woq_gemm_unit_top.power.fsdb");
        $fsdbDumpvars(0, tb_VX_woq_gemm_unit_top, "+all");
        $fsdbDumpMDA();
    end
