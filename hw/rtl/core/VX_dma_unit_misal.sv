...
//    3. tagged response capture and generalized destination alignment
...
`endif
  ...
  localparam int GEN_OFFSET_W       = `LOG2UP(MIN_BYTES);
...
`endif
  ...
  logic [RD_OUTSTANDING-1:0] slot_segment_eop_r;
  logic [RD_OUTSTANDING-1:0][GEN_OFFSET_W-1:0] slot_src_offset_r;
  ...
  wire [RD_SLOT_BITS-1:0] response_payload_raddr
      = (slot_retire && wr_next_slot_ready)
  ...
  wire gen_dcache_wr_valid;
  wire gen_dcache_wr_ready = dcache_req_ready_w;
  wire [DCACHE_BYTES*8-1:0] gen_dcache_wr_data;
  wire [DCACHE_BYTES-1:0] gen_dcache_wr_byteen;
  wire gen_dcache_wr_eop;
  wire gen_lmem_wr_valid;
  wire gen_lmem_wr_ready = lmem_req_ready_w;
  wire [LMEM_BYTES*8-1:0] gen_lmem_wr_data;
  wire [LMEM_BYTES-1:0] gen_lmem_wr_byteen;
  wire gen_lmem_wr_eop;
  wire gen_slot_ready;
  // The final destination EOP may precede the last physical gearbox slice.
  // Keep an already-draining source slot live after WR_DONE so its remaining
  // irrelevant slices can reach slot_ready and release the occupancy entry.
  wire gen_slot_valid = (state == S_RUN) && wr_slot_draining;
  wire gen_dst_eop = active_dir ? gen_dcache_wr_eop : gen_lmem_wr_eop;
  wire gen_dst_valid = active_dir ? gen_dcache_wr_valid : gen_lmem_wr_valid;
  wire gen_dst_ready = active_dir ? gen_dcache_wr_ready : gen_lmem_wr_ready;
  ...
  assign slot_retire = gen_slot_valid && gen_slot_ready;
  wire dst_req_issue_fire = gen_dst_fire;

  VX_dma_misal_gen_path #(
    ...
    .slot_valid           (gen_slot_valid),
    .slot_ready           (gen_slot_ready),
    .slot_data            (response_payload_rdata),
    .slot_lane            (32'(slot_lane_r[wr_expect_slot_r])),
    .slot_bytes           (32'(slot_remaining_r[wr_expect_slot_r])),
    .slot_segment_eop     (slot_segment_eop_r[wr_expect_slot_r]),
    .slot_src_offset      (slot_src_offset_r[wr_expect_slot_r]),
    .seg_size             (seg_size_r),
    .valid_total          (valid_total),
    .zero_segment_start   (gen_zero_segment_start),
    .dcache_dst_offset    (`LOG2UP(DCACHE_BYTES)'(wr_dst_seg_base_r)),
    .lmem_dst_offset      (`LOG2UP(LMEM_BYTES)'(wr_dst_seg_base_r)),
    .dcache_wr_valid      (gen_dcache_wr_valid),
    .dcache_wr_ready      (gen_dcache_wr_ready),
    .dcache_wr_data       (gen_dcache_wr_data),
    .dcache_wr_byteen     (gen_dcache_wr_byteen),
    .dcache_wr_eop        (gen_dcache_wr_eop),
    .lmem_wr_valid        (gen_lmem_wr_valid),
    .lmem_wr_ready        (gen_lmem_wr_ready),
    .lmem_wr_data         (gen_lmem_wr_data),
    .lmem_wr_byteen       (gen_lmem_wr_byteen),
    .lmem_wr_eop          (gen_lmem_wr_eop)
  );
  ...
  assign response_payload_read = (state == S_RUN) && (wr_state == WR_RUN)
      && (wr_slot_ready || (slot_retire && wr_next_slot_ready));

  wire [DCACHE_BYTES-1:0] gen_dcache_rd_byteen;
  wire [LMEM_BYTES-1:0] gen_lmem_rd_byteen;
  for (genvar byte_idx = 0; byte_idx < DCACHE_BYTES; ++byte_idx) begin : g_dcache_rd_mask
    assign gen_dcache_rd_byteen[byte_idx]
        = (32'(byte_idx) >= rd_issue_lane)
       && (32'(byte_idx) < (rd_issue_lane + rd_issue_bytes));
  end
  for (genvar byte_idx = 0; byte_idx < LMEM_BYTES; ++byte_idx) begin : g_lmem_rd_mask
    assign gen_lmem_rd_byteen[byte_idx]
        = (32'(byte_idx) >= rd_issue_lane)
       && (32'(byte_idx) < (rd_issue_lane + rd_issue_bytes));
  end
  ...
  always_comb begin
    ...
    // read responses feed the aligner. Drain both response channels so an
    ...
    if (rd_can_issue) begin
      if (active_dir) begin
        ...
        lmem_req_byteen_w = gen_lmem_rd_byteen;
        ...
        dcache_req_byteen_w = gen_dcache_rd_byteen;
        dcache_req_tag_uuid_w = dma_uuid;
        dcache_req_tag_value_w = DCACHE_TAG_VALUE_W'(rd_issue_slot_r);
      end
    end

    if ((state == S_RUN) && (wr_state == WR_RUN) && gen_dst_valid) begin
      if (active_dir) begin
        dcache_req_valid_w = gen_dcache_wr_valid;
        dcache_req_rw_w = 1'b1;
        dcache_req_addr_w = to_dcache_addr(wr_dst_write_base_r);
        dcache_req_data_w = gen_dcache_wr_data;
        dcache_req_byteen_w = gen_dcache_wr_byteen;
        dcache_req_tag_uuid_w = dma_uuid;
      end else begin
        lmem_req_valid_w = gen_lmem_wr_valid;
        lmem_req_rw_w = 1'b1;
        lmem_req_addr_w = to_lmem_addr(wr_dst_write_base_r);
        lmem_req_data_w = gen_lmem_wr_data;
        lmem_req_byteen_w = gen_lmem_wr_byteen;
        lmem_req_tag_uuid_w = dma_uuid;
      end
    end
  end
  ...
  wire [SLOT_OCC_W-1:0] slot_occupancy_next = slot_occupancy_r
      + SLOT_OCC_W'(src_req_issue_fire) - SLOT_OCC_W'(slot_retire);
  ...
  always_ff @(posedge clk) begin
    if (reset) begin
      ...
      for (int s = 0; s < RD_OUTSTANDING; ++s) begin
        ...
        slot_segment_eop_r[s] <= 1'b0;
        slot_src_offset_r[s] <= '0;
      end
    end else begin
      state <= state_n;
      ...
      if ((state == S_PRECALC) && precalc_done
        ...
        if (src_req_issue_fire) begin
          ...
          slot_segment_eop_r[rd_issue_slot_r] <= rd_crosses_seg;
          slot_src_offset_r[rd_issue_slot_r]
              <= GEN_OFFSET_W'(rd_src_seg_base_r);
        ...
        if (slot_retire) begin
          slot_state_r[wr_expect_slot_r] <= SLOT_FREE;
          slot_lane_r[wr_expect_slot_r] <= '0;
          slot_remaining_r[wr_expect_slot_r] <= '0;
          slot_segment_eop_r[wr_expect_slot_r] <= 1'b0;
          slot_src_offset_r[wr_expect_slot_r] <= '0;
          wr_expect_slot_r <= wr_next_slot_idx;
        end

        if (gen_dst_fire) begin
          if (gen_dst_eop) begin
            ...
            wr_out_off_r <= wr_out_off_r + wr_dst_bytes;
          end
        end
...
`ifdef SIMULATION
  always_ff @(posedge clk) begin
    ...
    if (!reset && slot_retire) begin
      assert (slot_state_r[wr_expect_slot_r] == SLOT_DRAINING)
        else $fatal(1, "%m: generalized source retired from non-draining slot %0d",
                    wr_expect_slot_r);
    end
  end
`endif
...
`ifdef DBG_TRACE_GEMM
  always_ff @(posedge clk) begin
    if (!reset) begin
      if (cmd_start) begin
        `TRACE(2, ("%m : [%0t] | DMA_START | {inst=%s, entry_id=%0d, dir=%0d, src_base=0x%0h, dst_base=0x%0h, seg_size=%0d, padding=%0d, bound=[%0d,%0d,%0d], slots=%0d}\n",
      ...
      if (dst_req_issue_fire) begin
        `TRACE(3, ("%m : [%0t] | DMA_WR_ENQUEUE_GEN | {inst=%s, addr=0x%0h, eop=%0d}\n",
                  $time, INSTANCE_ID, wr_dst_write_base_r, gen_dst_eop))
      end
    end
  end
`endif
