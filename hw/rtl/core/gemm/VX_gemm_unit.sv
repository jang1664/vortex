...
module VX_gemm_unit import VX_gpu_pkg::*; #(
    ...
    logic [`GEMM_SCALE_ZERO_DATA_SIZE-1:0]           sz_req_data;
    logic                                            scale_reg0_wr_en, scale_reg1_wr_en;
    logic                                            zp_reg0_wr_en, zp_reg1_wr_en;
    logic [$clog2(`MAX(`MXU_ROW, `MXU_COL))-1:0]     sz_reg_idx;
    ...
    always_comb begin
        ...
        if (sz_req_valid & sz_req_rw) begin
            if (sz_req_addr >= SCALE_REG0_BASE && sz_req_addr < SCALE_REG1_BASE) begin
                scale_reg0_wr_en = 1'b1;
                sz_reg_idx       = (sz_req_addr - SCALE_REG0_BASE) / (`SCALE_WIDTH/8);
            end else if (sz_req_addr >= SCALE_REG1_BASE && sz_req_addr < ZP_REG0_BASE) begin
                scale_reg1_wr_en = 1'b1;
                sz_reg_idx       = (sz_req_addr - SCALE_REG1_BASE) / (`SCALE_WIDTH/8);
            end else if (sz_req_addr >= ZP_REG0_BASE && sz_req_addr < ZP_REG1_BASE) begin
                zp_reg0_wr_en    = 1'b1;
                sz_reg_idx       = (sz_req_addr - ZP_REG0_BASE) / (`ZP_WIDTH/8);
            end else if (sz_req_addr >= ZP_REG1_BASE) begin
                zp_reg1_wr_en    = 1'b1;
                sz_reg_idx       = (sz_req_addr - ZP_REG1_BASE) / (`ZP_WIDTH/8);
            end
        end
    end
    ...
    always_ff @(posedge clk, posedge reset) begin
        if (reset) begin
            ...
            if (scale_reg0_wr_en) begin
                scale_regs[0][sz_reg_idx] <= sz_req_data[`SCALE_WIDTH-1:0];
            end
            if (scale_reg1_wr_en) begin
                scale_regs[1][sz_reg_idx] <= sz_req_data[`SCALE_WIDTH-1:0];
            end
        end
    end
    ...
    always_ff @(posedge clk, posedge reset) begin
        if (reset) begin
            ...
            if (zp_reg0_wr_en) begin
                zero_regs[0][sz_reg_idx] <= sz_req_data[`ZP_WIDTH-1:0];
            end
            if (zp_reg1_wr_en) begin
                zero_regs[1][sz_reg_idx] <= sz_req_data[`ZP_WIDTH-1:0];
            end
        end
    end
    ...
    generate
        for (genvar i = 0; i < `MXU_ROW; i++) begin : gen_in_scaler
            ...
            assign activated = (gemm_unit_ctrl.quant_dir == `QDIR_COL) & in_flight;
