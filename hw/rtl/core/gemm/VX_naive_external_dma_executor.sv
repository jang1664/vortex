// Naive metadata DMA child; descriptor programming retains the existing
// allocator/poll protocol. Only real LOAD/STORE commands are executable.
...
module VX_naive_external_dma_executor import VX_gpu_pkg::*; #(
    ...
    input wire [31:0] M_orig, N_orig, K_orig, qblk_orig,
    input wire [31:0] M_target, N_target, K_target,
  ...
  // ============================================================
  // ============================================================
  typedef enum logic [2:0] {T_INPUT, T_WEIGHT, T_OUTPUT, T_SCALE, T_ZP, T_INVALID} tensor_t;
  tensor_t tensor_sel;
  ...
  assign quiescent = (state_q == S_IDLE);
  assign done_valid = (state_q == S_DONE);
  assign done_work_seq = cmd_q.work_seq;
  ...
  always_comb begin
    ...
    unique case (state_q)
      S_IDLE: begin
        poll_gap_d  = 0;
        alloc_gap_d = 0;
        if ((cmd_valid && cmd_ready)) state_d = S_DECODE;
      end
  ...
  always_ff @(posedge clk) begin
    if (reset) begin
      ...
      if (state_q == S_IDLE && (cmd_valid && cmd_ready)) begin
`ifndef SYNTHESIS
        assert ((cmd.instr[3:0] == OP_DMA_LD || cmd.instr[3:0] == OP_DMA_ST)
             && cmd.work_seq != 0 && cmd.rd <= 4)
            else $fatal(1,"%s invalid external DMA command",INSTANCE_ID);
`endif
        ...
        N_orig_q      <= N_orig;
        K_orig_q      <= K_orig;
        qblk_orig_q   <= qblk_orig;
        M_target_q    <= M_target;
        N_target_q    <= N_target;
        K_target_q    <= K_target;
        wtrans_tot_q  <= wtrans_tot;
        qdir_tot_q    <= qdir_tot;
      end
    end
  end
