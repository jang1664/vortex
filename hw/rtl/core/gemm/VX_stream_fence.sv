...
module VX_stream_fence #(
    ...
    VX_stream_intf.sink   push_i[NB_STREAMS],
    VX_stream_intf.source pop_o [NB_STREAMS]
);
  ...
  generate
    for (genvar ii = 0; ii < NB_STREAMS; ii++) begin : binding
      ...
      assign in_ready[ii] = push_i[ii].ready;
      ...
      always_ff @(posedge clk_i or negedge resetn_i) begin
        if (~resetn_i) data_q[ii] <= '0;
        else if (clear_i) data_q[ii] <= '0;
        else if ((in_valid[ii] & in_ready[ii]) && ~fence_state_q[ii]) data_q[ii] <= push_i[ii].data;
      end

      always_ff @(posedge clk_i or negedge resetn_i) begin
        if (~resetn_i) strb_q[ii] <= '0;
        else if (clear_i) strb_q[ii] <= '0;
        else if ((in_valid[ii] & in_ready[ii]) && ~fence_state_q[ii]) strb_q[ii] <= push_i[ii].strb;
      end
  ...
  always_comb begin
    fence_state_d = '0;
    out_valid = 1'b0;
    if (&((in_valid & in_ready) | fence_state_q)) out_valid = 1'b1;
    else fence_state_d = fence_state_q | (in_valid & in_ready);
  end

  always_ff @(posedge clk_i or negedge resetn_i) begin
    if (~resetn_i) fence_state_q <= '0;
    else if (clear_i) fence_state_q <= '0;
    else fence_state_q <= fence_state_d;
  end
