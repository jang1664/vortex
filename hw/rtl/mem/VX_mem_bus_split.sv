...
module VX_mem_bus_split import VX_gpu_pkg::*; #(
    ...
    // Preserve the legacy path outside naive builds.
`ifndef GEMM_NAIVE
    `VX_STATIC_ASSERT(!RSP_REORDER, ("response reordering requires GEMM_NAIVE"))
`endif
...
`ifdef DBG_TRACE_MEM
    always @(posedge clk) begin
        if (!reset) begin
            if (context_push) begin
                `TRACE(1, ("%t: %s %m read slot=%0d tag=0x%0h mask=%b\n",
                    $time, INSTANCE_ID, write_slot_r, original_tag, request_mask))
            end
            if (read_payload) begin
                `TRACE(1, ("%t: %s %m assembled slot=%0d tag=0x%0h\n",
                    $time, INSTANCE_ID, read_slot_r, head_tag))
            end
        end
    end
`endif
