    ...
    if (WRITE_ENABLE) begin : g_mreq_queue
        if (WRITEBACK) begin : g_wb
            if (DIRTY_BYTES) begin : g_dirty_bytes
                ...
                `VX_RUNTIME_ASSERT (~do_fill_or_flush_st1 || (is_dirty_st1 == has_dirty_bytes), ("%t: missmatch dirty bytes: dirty_line=%b, dirty_bytes=%b, addr=0x%0h", $time, is_dirty_st1, has_dirty_bytes, `CS_BANK_TO_FULL_ADDR(addr_st1, BANK_ID)))
            end
            ...
            // issue a memory write on a write request, but don't resend on replay
...
`ifdef DBG_TRACE_CACHE
    ...
    if (WRITE_ENABLE && !WRITEBACK) begin : g_wt_store_track
        ...
        wire wt_store_emit = mreq_queue_push && do_write_st1 && ~is_replay_st1;
        ...
        always @(posedge clk) begin
            if (reset) begin
                ...
                if (wt_store_emit && (wt_store_pending == 0)) begin
                    $error("%t: *** %s wt-store underflow: mreq_addr=0x%0h, mreq_tag=0x%0h, core_tag_st1=0x%0h",
                        $time, INSTANCE_ID, mreq_queue_full_addr, mreq_queue_tag, tag_st1);
                end
