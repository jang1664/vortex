...
`ifndef PLATFORM_MEMORY_NUM_PORTS
`ifdef PLATFORM_MERGED_MEMORY_INTERFACE
`define PLATFORM_MEMORY_NUM_PORTS 1
`else
`define PLATFORM_MEMORY_NUM_PORTS `PLATFORM_MEMORY_NUM_BANKS
`endif
`endif
...
`ifndef L1_MEM_PORTS
`ifdef L1_DISABLE
`define L1_MEM_PORTS `MIN(DCACHE_NUM_REQS, `PLATFORM_MEMORY_NUM_PORTS)
`else
`define L1_MEM_PORTS `MIN(`DCACHE_NUM_BANKS, `PLATFORM_MEMORY_NUM_PORTS)
`endif
`endif
...
`ifndef L2_MEM_PORTS
`ifdef L2_ENABLE
`define L2_MEM_PORTS `MIN(`L2_NUM_BANKS, `PLATFORM_MEMORY_NUM_PORTS)
`else
`define L2_MEM_PORTS `MIN(L2_NUM_REQS, `PLATFORM_MEMORY_NUM_PORTS)
`endif
`endif
...
`ifndef L3_MEM_PORTS
`ifdef L3_ENABLE
`define L3_MEM_PORTS `MIN(`L3_NUM_BANKS, `PLATFORM_MEMORY_NUM_PORTS)
`else
`define L3_MEM_PORTS `MIN(L3_NUM_REQS, `PLATFORM_MEMORY_NUM_PORTS)
`endif
`endif
...
`endif
`define MXU_WLOAD_NUM 1           // Number of weight loads per cycle
...
`define GEMM_WEIGHT_DATA_SIZE     ((`MXU_COL*`MXU_WLOAD_NUM*`W_BIT_WIDTH)/8) // 16 bytes (1*32 INT4 weights)
